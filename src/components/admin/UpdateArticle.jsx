...
const UpdateArticle = () => {
  ...
  // FETCHING THE ARTICLE DETAILS ON COMPONENT MOUNT
  useEffect(() => {
    // LOADING STATE
    setLoading(true);
    axios
      .get(`${ARTICLE_API_ENDPOINT}/slug/${slug}`, { withCredentials: true })
      .then((r) => {
        const article = r.data.data;
        // SETTING DATA IN THE FROM
        setTitle(article.title);
        setBannerFile({ preview: article.bannerUrl, isNew: false });
        setSelectedCategories(
          article.categories.map((cat) => ({ label: cat, value: cat }))
        );
        setSections(
          article.sections.flat().map((sec) => ({
            heading: sec.heading,
            content: sec.content,
            originalUrl: sec.imageUrl || null,
            file: null,
            preview: sec.imageUrl || null,
          }))
        );
  ...
  const onSectionImageChange = (idx, e) => {
    ...
    setSections((s) => {
      ...
      copy[idx] = {
        ...
        file,
        preview: URL.createObjectURL(file),
        originalUrl: copy[idx].originalUrl,
      };
      return copy;
    });
  };
  ...
  const submitHandler = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `${ARTICLE_API_ENDPOINT}/${articleId}/update`,
        formData,
      ...
      console.error(
        error?.response?.data?.message || "Error Updating Article!"
      );
      // TOASTING ERROR MESSAGE
        ...
        <section className="flex flex-col items-center justify-center gap-[2rem] w-full border-2 border-gray-100 rounded-xl sm:p-[2rem] p-[1rem] tracking-[0.5px]">
          ...
          <div className="w-full flex items-center justify-between flex-wrap-reverse gap-[1rem]">
            <h1 className="flex items-center gap-2 font-[600] text-gray-500 text-[2rem]">
              <Edit className="text-color-DB w-[3rem] h-[3rem]" /> Update
              Article
            </h1>
            <Button
